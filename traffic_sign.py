...
while True:
    ...
    imt=cv2.cvtColor(imgOrignal,cv2.COLOR_BGR2GRAY)
    cnt=croping(imt.astype('uint8'))
    (x,y,w,h)=cv2.boundingRect(cnt)
    imgO=imgOrignal[y:y+h,x:x+w]
    cv2.imshow("cropped",imgO)
    img = np.asarray(imgO)
